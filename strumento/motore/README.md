...
- **l'app web**, [`libro/santa-rita.html`](libro/santa-rita.html): un solo file HTML con dentro motore, dati e interfaccia, da aprire in qualunque browser. Ha un panorama del porto disegnato in tempo reale, con la luce che cambia col momento del giorno. Il quadro è un foglio della capitaneria con le quattro domande e la barra delle fasce, su cui rotolano i dadi e cadono due timbri: la fascia e il grado. Il *Quasi* compare fra le scelte, ciascuna con il suo costo. Quando c'è un compagno, il foglio ha tre linguette (da solo, con il suo aiuto, lui al posto tuo) e il quadro si ricalcola per ciascuna. La scheda mostra tratti, compagni, persone, convinzioni e il taccuino con lo stato di ogni notizia. Il pannello «Sotto il cofano» mostra i dati grezzi che il motore restituisce: l'app li disegna e basta;
...
`vista` restituisce il testo della scena, lo stato del personaggio (compresi i compagni) e le scelte, comprese quelle chiuse con ciò che manca e il motivo (`chiusaDa`: un requisito, una convinzione, la forza, una ferita, la difficoltà). Ogni prova porta il suo `quadro`, con le quattro sezioni: `puoi`, `riesci` (righe, soglia, fasce in percentuale e in trentaseiesimi), `costo` (righe e grado), `dopo`. Se c'è un compagno che conosce la capacità, la prova porta le sue `varianti`: `id@aiuto:lucia` e `id@fa:lucia`, ciascuna con il suo quadro; si passano ad `agisci` come le altre scelte. Quando una prova finisce nel *Quasi*, la vista successiva ha `sospeso` e offre soltanto le possibilità del *Quasi*, ciascuna con il suo `costa`. `agisci` restituisce una partita nuova (quella vecchia non cambia) e la lista degli eventi: tiri, esiti con la fascia e il grado, notizie, persone, tratti, Traccia, ferite, imprevisti. Un gioco grafico può mostrare gli eventi come animazioni e il quadro come un pannello (§48.4). Ogni scena può portare un campo `presentazione` con indicazioni libere per la grafica, che il motore ignora.

## La storia di prova

*Il registro della Santa Rita*: cinquantanove scene nel porto dell'appendice B. Il fratello di chi gioca è sparito, e la nave su cui lo tengono prigioniero salpa prima dell'alba. Il personaggio ha quattro tratti (*Vista acuta*, *Sa leggere*, *Minuto*, *Soffre il mare*), due convinzioni, e conosce già l'oste Zeno. Nella storia ci sono vie che si aprono solo per chi è fatto in un certo modo, notizie false che si smentiscono, una convinzione che si può lasciare andare e che apre la strada al capitano Serra, una deduzione, una lanterna da accendere, persone che ricordano, Lucia che può venire con te come compagna, un confronto a parole con un archivista (*chi si chiude*) e uno col coltello con un sicario (*chi avanza*). Si può morire, ma solo dopo averlo letto nella posta.

Il personaggio non sa nuotare: da solo non può entrare in acqua. Lucia sì, e con lei le vie dell'acqua si aprono. In ogni prova in cui conosce la capacità, il gioco offre di farsi aiutare da lei (+1) o di lasciarla fare al posto tuo, con il suo livello e i suoi tratti.
